...
import { getUserPoints, getLeaderboard } from './gamification.service';
...
router.get('/points/:communityId', requireAuth, async (req: Request, res: Response) => {
  try {
...
router.get('/activity/:communityId', requireAuth, async (req: Request, res: Response) => {
  try {
...
router.get('/levels/:communityId', async (req: Request, res: Response) => {
  try {
    const levels = await prisma.level.findMany({
      where: { communityId: param(req, 'communityId') },
      orderBy: { minPoints: 'asc' },
