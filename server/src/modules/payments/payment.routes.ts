...
router.post('/initiate', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const flouciData = await flouciResponse.json() as any;
...
router.post('/verify/:paymentId', requireAuth, async (req: Request, res: Response) => {
  try {
...
router.get('/earnings/:communityId', requireAuth, async (req: Request, res: Response) => {
  try {
