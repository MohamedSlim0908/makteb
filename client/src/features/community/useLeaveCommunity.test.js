...
describe('useLeaveCommunity', () => {
  it('calls DELETE /communities/:id/leave', async () => {
    api.delete.mockResolvedValue({ data: { success: true } });

    const { result } = renderHook(() => useLeaveCommunity('c1', 'my-slug'), {
    ...
    expect(api.delete).toHaveBeenCalledWith('/communities/c1/leave');
  });
  ...
  it('invalidates membership and community queries on success', async () => {
    ...
    const { result } = renderHook(() => useLeaveCommunity('c1', 'my-slug'), { wrapper });
