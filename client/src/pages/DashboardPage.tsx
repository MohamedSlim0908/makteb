...
export function DashboardPage() {
  ...
  const { data: communitiesData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get<{ data: Community[] }>(`/communities?${params}`);
      return data;
    },
  });
  ...
  const { data: coursesData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<{ data: Course[] }>(`/communities/${activeCommunityId}/courses`);
      return data;
    },
  ...
  const { data: earnings } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Earnings>(`/payments/earnings/${activeCommunityId}`);
      return data;
    },
    enabled: !!activeCommunityId,
  });
  ...
  const createCourseMutation = useMutation({
    mutationFn: (body: { title: string; description?: string; price?: number }) =>
