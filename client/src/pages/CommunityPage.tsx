...
type TabId = 'feed' | 'courses' | 'leaderboard' | 'about';
...
export function CommunityPage() {
  ...
  const { data: posts, isLoading: postsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Post[]>(`/posts/community/${community!.id}`);
      return data;
    },
  ...
  const { data: courses, isLoading: coursesLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Course[]>(`/courses/community/${community!.id}`);
      return data;
    },
  ...
  const { data: leaderboard, isLoading: leaderboardLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<LeaderboardEntry[]>(`/gamification/leaderboard/${community!.id}`);
      return data;
    },
  ...
  const { data: members, isLoading: membersLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Member[]>(`/communities/${community!.id}/members`);
      return data;
    },
